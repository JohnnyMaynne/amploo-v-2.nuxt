<template>
  <div class="px-3 relative inline-block text-left">
      <nuxt-link to="profile" type="button" :class="[active ? 'bg-white' : 'hover:bg-gray-200']" class="group w-full bg-gray-100 rounded-md px-3.5 py-2 text-sm text-left font-medium text-gray-700  focus:outline-none" aria-expanded="false" aria-haspopup="true">
...
export default {
  computed: {
    active() {
      return 'profile' === this.$route.name
    },
  }
}
</script>
